...
-- 1) Genes with more than one promoter
SELECT gene_id
FROM gene_promoter
GROUP BY gene_id
HAVING COUNT(promoter_id) > 1;

-- 2) Protein-coding genes related to kinases (based on description text)
SELECT gene_name, description, gene_type
FROM gene
WHERE description LIKE '%kinase%'
  AND gene_type = 'protein_coding';

-- 3) Genes with more than one transcript
SELECT gene_id
FROM transcript
GROUP BY gene_id
HAVING COUNT(transcript_id) > 1;

-- 4) Genes with variants located inside their promoters
...
-- 5) Genes with variants located inside transcript regions (coding-region proxy)
SELECT DISTINCT g.gene_id, g.gene_name
FROM gene g
JOIN gene_variant gv ON g.gene_id = gv.gene_id
JOIN variant v ON gv.variant_id = v.clinvar_id
